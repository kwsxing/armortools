...
void *io_fbx_parse_v2(char *buf, size_t size, const char *path, const io_mesh_config_t *const io_mesh_cfg, io_mesh_progress_callback progress_callback) {
	...
	ufbx_scene    *scene = ufbx_load_memory(buf, size, &opts, NULL);
	if (scene != NULL) {
		any_array_t *meshes = any_array_create(scene->nodes.count);
		memset(meshes->buffer, 0, sizeof(raw_mesh_t *) * scene->nodes.count);
		...
		for (uint32_t i = 0; i < scene->nodes.count; ++i) {
			...
			raw_mesh_t *part = (raw_mesh_t *)calloc(sizeof(raw_mesh_t), 1);
			memset(part, 0, sizeof(raw_mesh_t));

			io_fbx_parse_mesh(part, n->mesh, &n->node_to_world, &n->unscaled_node_to_world);

			if (part->posa != NULL) {
				part->name = malloc(strlen(n->name.data) + 1);
				strcpy(part->name, n->name.data);
